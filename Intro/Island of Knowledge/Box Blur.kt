...
that's about to go viral! You can't let this ruin your reputation,
so you want to apply the box blur algorithm to the photo to hide its content.

The pixels in the input image are represented as integers.
The algorithm distorts the input image in the following way:
 Every pixel x in the output image has a value equal to the average value of
 the pixel values from the 3 × 3 square that has its center at x, including x itself.
...
There are four 3 × 3 squares in the input image, so there should be four integers in the blurred output.
 To get the first value: (7 + 4 + 0 + 5 + 6 + 2 + 6 + 10 + 7) = 47 / 9 = 5.2222 = 5.
  The other three integers are obtained the same way, then the surrounding integers are
...
fun boxBlur(image: MutableList<MutableList<Int>>): MutableList<MutableList<Int>> {
    val myImage = mutableListOf<MutableList<Int>>()
    for (i in 2..image.lastIndex) {
        myImage.add(mutableListOf())
        for (j in 2..image[0].lastIndex)
            myImage[i - 2].add(image.slice(i - 2..i).sumBy { it.slice(j - 2..j).sum() } / 9)
    }
    return myImage
}
