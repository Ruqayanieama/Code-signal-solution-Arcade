...
times as the previous letter in the alphabet within the string; ie: b occurs no more times than a;
...
fun isBeautifulString(inputString: String): Boolean {
    ...
    for (i in 0 until inputString.length) {
        alpha[inputString[i].toInt() - 97]++
    }
    for (j in 1 until alpha.size) {
