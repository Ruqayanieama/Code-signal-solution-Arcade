...
The third candidate can win even if none of the remaining candidates vote for him. For example,
 if each of the remaining voters cast their votes for each of his opponents, he will still be the winner
...
fun electionsWinners(votes: MutableList<Int>, k: Int): Int {
    val i = votes.max()!! - k
    var j = 0
    if (k != 0) j = votes.count {it > i}
    else if (votes.count {it == i} <= 1) j = votes.count {it == i}
    return j
}
